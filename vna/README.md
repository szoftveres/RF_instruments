...
Since the 3-term model expects perfect termintaion on all other ports for multi-port multilateral networks (passive filters, attenuators, etc..), a precise S1,1 measurement requires the other terminal (of e.g. a filter) to be momentariy disconnected from Port 2 and terminated by a good quality load (e.g. the load cal standard). This is not much of an issue as long as Port 2 is well isolated (e.g. uni-lateral networks, amplifiers, antennas, etc..).
