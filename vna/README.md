...
Hobby-level low-cost vector network analyzers (like the LiteVNA) are capable of reaching higher frequencies, but the inability to decrease the RF power level precludes any measurement on active small-signal devices, like LNAs, RFICs and active mixers. The RF output level of these basic VNAs is at around 0 dBm, which drives active devices into their non-linear / compression region. While technically it's possible to decrease the power level externally, a passive attenuator placed after Port 1 also attenuates the reflected waves, which results in a dramatic loss of (already limited) dynamic range of the S1,1 measurement, making the input matching / tuning and characterization of small-signal active devices practically impossible.

A programmable RF attenuator placed at the RF source of the VNA makes the RF level adjustable internally (i.e. also affecting  the reference path of the VNA), so theoretically the VNA always stays calibrated even if the RF power level is changed on the fly (this is not entirely true in practice, due to the internal non-linearities of the VNA). Since the reflected (S11) path now doesn't suffer from double attenuation, as it would with an external attenuator, small-signal reflection measurements can be made with high dynamic range, as well as power sweeps.
...
The reflected (S1,1) error correction is based on the well known 3-term error model (implementation [here](https://github.com/szoftveres/RF_Microwave/tree/main/RFlib/p1cal.m)). I'm using a simple DIY SMA cal kit and treating them as perfect standards (reflection coefficients for the open- short and load are 1, -1 and 0 respectively, at all frequencies), which is far from ideal; the GNU Octave based [processing and UI software](https://github.com/szoftveres/RF_Microwave/tree/main/instrctl/vna_gui.m) is capable of accepting cal kit parameters in a HP-Agilent-Keysight format, therefore the VNA can be calibrated with high-quality cal kits.

The 3-term error correction expects perfect termintaion of a multi-port multilateral network on every port, so a precise S1,1 measurement requires the other port of such network (e.g. a passive, bidirectional filter) to be momentariy disconnected from Port 2 (whose termination is not perfect) and terminated by a good quality load (e.g. the load cal standard). This is not much of an issue with uni-lateral two-port networks where the reverse path from Port 2 is well isolated (e.g. amplifiers with good reverse isolation, or an output attenuator calibrated into S2,1) or not involved at all (e.g. S11 measurement of antennas).
...
GNU Octave has some built-in functions and widgets that can be used to create a simple GUI; the benefit is that (just like with Python) the resulting software can run on most major OS platforms (Linux, Windows, Mac); plus the same widget and UI toolkits are also available in Matlab. The [UI software](https://github.com/szoftveres/RF_Microwave/tree/main/instrctl/vna_gui.m) implements the most essential functions and configuration options necessary for using this VNA in a fully functional continuous sweep mode:
